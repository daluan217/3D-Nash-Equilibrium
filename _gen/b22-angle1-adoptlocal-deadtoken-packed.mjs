...
async function registerAndLogin(username, email, password, { dismissOffer = true } = {}) {
  ...
  if (dismissOffer) {
    const offer = page.locator('text=/saved on this device/i');
...
const offer = page.locator('text=/saved on this device/i');
// locator.isVisible() is an INSTANT check in Playwright — its `timeout` option
// does not make it wait. The offer is set from the login response inside a
// React state update, so an instant check right after the response races the
// render and reports false for a dialog that does appear. waitFor() is the
// call that actually waits. Behaviour asserted below is unchanged.
let offerVisible = true;
try { await offer.first().waitFor({ state: 'visible', timeout: 8000 }); }
catch { offerVisible = false; }
