...
try{
  ...
  const cmp = await insp.ev(`(()=>{
    ...
    const deps=[/const VERSION_RE = [^\\n]+/,/function isVersion\\([^\\n]+/,/function compareVersions[\\s\\S]*?\\n\\}/]
      .map(re=>{const x=re.exec(src); if(!x) throw new Error('MISSING DEP '+re); return x[0];});
    const f=new Function(deps.join('\\n')+'\\nreturn compareVersions;')();
    const cur='0.0.223';
    // POSITIVE CONTROL: if a plainly-newer version does not compare greater the
    // extraction is broken and every "offersUpdate:false" below is vacuous.
    if(!(f('9.9.9',cur)>0)) throw new Error('CONTROL FAILED: extracted comparator inert');
  ...
  const hostile = await insp.ev(`(async ()=>{
    ...
    for (const [label, shape] of shapes) {
      ...
      try {
        ...
        const cvs=[/const VERSION_RE = [^\\n]+/,/function isVersion\\([^\\n]+/,/function compareVersions[\\s\\S]*?\\n\\}/]
          .map(re=>{const x=re.exec(src); if(!x) throw new Error('MISSING DEP '+re); return x[0];}).join('\\n');
        const fn=new Function('app','dialog','shell','UPDATE_BASE_URL','EXTERNAL_URL_SCHEMES',
          cvs+';'+oes[0]+';'+m[0]+'; return checkForUpdates;')(
  ...
  try {
    const rows=JSON.parse(hostile);
    for (const r of rows) console.log(`   ${String(r.label).padEnd(24)} offered=${String(r.offered).padEnd(5)} openedWith=${r.openedWith} threw=${r.threw||'-'}`);
    // POSITIVE CONTROL: 'newer' (9.9.9 vs 0.0.223) is the one row that MUST be
    // offered. Without it every other offered=false is the harness failing, not
    // the product refusing — which is exactly how this matrix went vacuous.
    const ctl=rows.find(r=>r.label==='newer');
    if(!ctl || ctl.offered!==true || !String(ctl.openedWith||'').startsWith('https://'))
      console.log('   *** CONTROL FAILED: "newer" was not offered ->',JSON.stringify(ctl),'— matrix is VACUOUS');
    else console.log('   control ok: "newer" offered and opened', ctl.openedWith);
  }
