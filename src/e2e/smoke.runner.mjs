...
const results = [];
let activePage = null;
const pendingFailureHtml = [];
const sanitize = (name) => name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '').slice(0, 120);
function record(name, pass, detail) {
  const row = { name, pass, detail };
  if (!pass && activePage) {
    const htmlPath = path.join(artifactDir, `smoke-${sanitize(name)}.html`);
    pendingFailureHtml.push(activePage.content().then((html) => writeFileSync(htmlPath, html)).catch(() => {}));
    row.detail = detail ? `${detail} [html: ${htmlPath}]` : `[html: ${htmlPath}]`;
  }
  results.push(row);
  console.log(`${pass ? 'PASS' : 'FAIL'} ${name}${row.detail ? ' — ' + row.detail : ''}`);
}

// ── boot the production server (unless one is already listening) ────────────
let server = null;
const tmpRoot = path.join(process.cwd(), '.e2e-temp');
mkdirSync(tmpRoot, { recursive: true });
const userData = mkdtempSync(path.join(tmpRoot, 'nash-e2e-'));
...
try {
  ...
  await page.screenshot({ path: path.join(process.cwd(), 'playwright-artifacts', 'smoke_failure.png'), fullPage: true }).catch(() => {});
  try {
    writeFileSync(path.join(artifactDir, 'smoke_failure.html'),
...
await page.screenshot({ path: path.join(process.cwd(), 'playwright-artifacts', 'smoke_end.png') }).catch(() => {});
await browser.close();
...
await Promise.all(pendingFailureHtml);
