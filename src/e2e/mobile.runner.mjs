...
const PORT = process.env.E2E_PORT || process.env.MOBILE_PORT || '3097';
const BASE = process.env.E2E_BASE || process.env.MOBILE_BASE || `http://localhost:${PORT}`;
...
function record(name, pass, detail) {
  ...
  if (!pass && activePage) {
    const htmlPath = path.join(artifactDir, `mobile-${sanitize(name)}.html`);
...
const userData = mkdtempSync(path.join(tmpRoot, 'nash-mobile-'));
...
if (!(await waitReady())) {
  ...
  if (!(await waitReady())) { console.error('FAIL server never became ready'); await killServer(); throw new Error('server never became ready'); }
}
