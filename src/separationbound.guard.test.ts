...
for (const m of matches) {
  const [, name, kStr] = m;
  const k = Number(kStr);
  check(`${name}: multiplier ${k} clears the ~1x fragment-spread ceiling with margin (> 1.05)`, k > 1.05);
  check(`${name}: multiplier ${k} stays under the tightest genuine separation measured (~1.25x on CONTROL) (< 1.2)`, k < 1.2);
...
console.log(failures === 0
  ? '✓ separation-bound guard: both §71 pairwise-separation bounds are calibrated (diag * k, 1.05 < k < 1.2), not hard-coded'
